...
execute positioned ~ ~-50 ~ as @e[distance=..75,tag=!ui_temp_team,predicate=ui:load_unhurtable] unless score @s ui_obj_id matches -2147483648..2147483647 run function ui:misc/act/make_obj_id
execute positioned ~ ~-50 ~ as @e[distance=..75,tag=!ui_temp_team,predicate=ui:load_unhurtable,sort=furthest,limit=1] run tag @s add ui_temp_target2
...
# ミサイル召喚
    execute at @s run summon armor_stand ~ ~2 ~ {Tags:["tag","tmw_242"],Motion:[0.0d,0.0d,0.0d],Invisible:1b,Invulnerable:1b,Small:1b}
    #召喚した本体に固有ID設定
    execute as @e[tag=tag] at @s run function ui:misc/act/make_obj_id

    execute at @s run summon marker ~ ~2 ~ {Tags:["ui_temp_anchor"]}
    execute as @e[tag=ui_temp_anchor] run function ui:template/square_shuffle_2d
