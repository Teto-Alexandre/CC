...
function ui:common/particle/ss/downer/10102/l0/l0_0
...
execute if score $is2 ui_temp matches 2 if score $is ui_temp matches 3 run data merge storage ui:common {input:{Mode:"create",Var:-10103,Var2:2}}
execute if score $is2 ui_temp matches 2 if score $is ui_temp matches 3 run function ui:common/particle
...
# 爆風
    ...
    execute if score $is ui_temp matches 2 run scoreboard players set $damage ui_temp 120
    execute if score $is ui_temp matches 2 run execute at @e[distance=..1,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/projectile/hit.marker
