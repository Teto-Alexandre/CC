...
function ui:common/particle/ss/downer/10105/l1/l1_0
...
execute if score $is2 ui_temp matches 1 if score $is ui_temp matches 3 run data merge storage ui:common {input:{Mode:"create",Var:-10104,Var2:1}}
execute if score $is2 ui_temp matches 2 if score $is ui_temp matches 3 run data merge storage ui:common {input:{Mode:"create",Var:-10106,Var2:2}}
...
# 爆風
    ...
    execute if score $is ui_temp matches 2 run scoreboard players set $damage ui_temp 30
    execute if score $is ui_temp matches 2 run execute at @e[distance=..2.5,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/projectile/hit.marker
