...
execute if score $subtype ui_temp matches 464 run data modify storage ui:gun temp set value {Name:'{"text":"キューインキ","color":"gold"}',TimeUse:500,ShotType:1,ShotCT:1,BurstCT:15,Spread:0,AirSpread:0,SpreadType:1,AirSpreadType:1,Speed:35,SpeedPlus:15,RangeType:1,Range:50,FlyParticle:116,EndParticle:510,Damage:100,MultiShot:1,ShotFootStep:0,Sound:4,Pierce:1b,BulletData:{Explodion:{name:'{"text":"キューインキの爆風","color":"gold"}',damage:10,range:5.5,paint:5,command:"execute store result storage ui:gun temp3.BulletData.Explodion.damage run scoreboard players get @s ui_expdmg",sound:"playsound entity.firework_rocket.large_blast player @a ~ ~ ~ 2 0.75 0"}}}
