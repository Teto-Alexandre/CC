...
execute as @e[distance=0.01..9,tag=!ui_temp_team,predicate=ui:load_unhurtable,type=player] if score @s ui_obj_id = $obj_id ui_temp at @s positioned ~ ~ ~ run function ui:tmw/237/activator/time/464/drain.0.2
execute as @e[distance=0.01..9,tag=!ui_temp_team,predicate=ui:load_unhurtable,type=player] if score @s ui_obj_id = $obj_id ui_temp at @s positioned ~ ~ ~ run function ui:tmw/237/activator/time/464/drain.0.3
