...
#summon item ~ ~ ~ {Tags:["ui_temp_this","tmw_237_sub"],Invulnerable:1b,Item:{id:"minecraft:pumpkin",Count:1b,tag:{tmw:{type:1,color:1},CustomModelData:217111}},PickupDelay:32767s,Age:-32768s}
execute as @e[tag=ui_temp_this,limit=1] rotated as @p run teleport @s ~ ~ ~ ~ ~
execute as @e[tag=ui_temp_this,limit=1] rotated ~ ~-1 run function ui:template/accelerator/1
