#見た目
    ...
    function ui:template/sphere_particle/8
    scoreboard players set $damage_type ui_temp 3
    #summon zombie ~ ~ ~ {NoAI:1b,Glowing:1b}
    scoreboard players set $damage ui_temp 240
    execute at @e[distance=6.5..9,tag=!ui_temp_team,predicate=ui:load_unhurtable] run function ui:tmw/237/projectile/hit.marker
    scoreboard players set $damage ui_temp 600
