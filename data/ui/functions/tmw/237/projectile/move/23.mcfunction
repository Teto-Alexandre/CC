...
# 初期衝撃波
    ...
    # 弾にデータを込める
        execute if entity @s[tag=!tmw_237_proj_first] run data modify storage ui_temp: Unpower set value {Damage:0,Range:0,RangeType:1,Speed:900,FlyParticle:24,EndParticle:-1,Name:'',Pierce:1b}
...
# 見た目
    execute if score $team ui_temp matches 1 run function ui:tmw/237/projectile/move/23/1
    execute if score $team ui_temp matches 2 run function ui:tmw/237/projectile/move/23/2
    execute if score $team ui_temp matches 3 run function ui:tmw/237/projectile/move/23/3
    execute if score $team ui_temp matches 4 run function ui:tmw/237/projectile/move/23/4
