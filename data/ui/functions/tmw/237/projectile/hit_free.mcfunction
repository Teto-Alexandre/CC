...
# 命中処理ー消滅しない
    ...
    execute if score @s ui_bpart matches 12 as @e[tag=!ui_temp_team,predicate=ui:load_unhurtable,sort=nearest,limit=1] if entity @s[tag=!ui_resistance,nbt=!{Invulnerable:1b}] as @a if score @s ui_id = $id ui_temp at @s run scoreboard players operation $damage ui_temp = $damage.free ui_temp
