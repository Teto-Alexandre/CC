...
# チャージしているかに応じて変化
    ...
    execute if score $burst_alt.id ui_temp matches 303 if score $burst_alt ui_temp < $burst.max ui_temp store result score $ct ui_temp run data get storage ui:gun temp2.ShotCT