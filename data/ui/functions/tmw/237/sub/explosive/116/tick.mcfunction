...
# 処理終了
    ...
    execute as @s[tag=ui_237_sub_stop,tag=!ui_237_sub_first,tag=ui_237_thrown_by_module41] run effect give @e[tag=ui_temp_obj,tag=tmw_237_sub_116_1,limit=1,sort=nearest] resistance infinite 1 true
...
# 射撃
    execute if entity @s[tag=ui_237_sub_stop] run tag @e[distance=6..25,tag=!ui_temp_team,predicate=ui:load_unhurtable,nbt=!{active_effects:[{id:"minecraft:invisibility"}]},sort=nearest,limit=1] add ui_temp_target
