...
# 処理終了
    ...
    #ダメージ計算
    execute if entity @s[tag=ui_237_sub_first] store result score $temp ui_temp run data get entity @e[tag=ui_temp_obj,tag=tmw_237_sub_155_1,limit=1] Health 10
    ...
    execute if entity @s[tag=ui_237_sub_first] run data merge entity @e[tag=ui_temp_obj,tag=tmw_237_sub_155_1,limit=1] {Health:1000f}

# 射撃
    execute if entity @s[tag=ui_237_sub_stop] run function ui:tmw/237/sub/explosive/155/particle
    execute if entity @s[tag=ui_237_sub_stop] as @a[distance=..5,tag=ui_temp_team,predicate=ui:load_unhurtable] unless score @s energy_buff matches 1.. run function ui:tmw/237/sub/explosive/155/give

# 処理終了（消える）
    execute if entity @s[tag=ui_237_sub_explode] run function ui:tmw/237/sub/explosive/155/end
