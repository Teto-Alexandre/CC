...
# 常時実行
    ...
    execute if score $time ui_temp matches 0 run particle portal ~ ~1.5 ~ 0 0 0 2.5 60 force
    execute if entity @s[tag=ui_237_sub_stop] as @e[distance=..5.5,tag=tmw_237_sub] unless score @s ui_team = $team ui_temp run tag @s add ui_237_sub_explode
...
# 処理終了
    ...
    execute as @s[tag=ui_237_sub_stop] run scoreboard players remove @s ui_is 4
    ...
    execute as @s[tag=ui_237_sub_stop,tag=!ui_237_sub_first] run data merge storage ui:common {input:{Mode:"create",Var:-10023,Var2:2}}
