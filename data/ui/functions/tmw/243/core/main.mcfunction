# メニュー表示
    ...
    execute if score $success ui_temp matches 0 if entity @s[tag=!setting_module,tag=setting_game,tag=setting_party] run function ui:tmw/243/core/panel/party
    execute if score $success ui_temp matches 0 if entity @s[tag=!setting_module,tag=setting_game,tag=!setting_party] run function ui:tmw/243/core/panel/setting
