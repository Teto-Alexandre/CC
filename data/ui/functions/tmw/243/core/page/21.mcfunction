...
# スロットが空白ならボタンが押されたと判断する＋changedを1に
    ...
    execute if score $slot ui_temp matches 3 run scoreboard players set $sub ui_temp 1013
    execute if score $slot ui_temp matches 3 run scoreboard players set $sp ui_temp 1014
...
# 名前を入力
    ...
    execute if score $slot ui_temp matches 3 run data modify storage ui:gun temp.tmw.gun.SubName set value "カウンター"
    execute if score $slot ui_temp matches 3 run data modify storage ui:gun temp.tmw.gun.SPName set value "カウンター"
