...
# 名前を入力
    ...
    execute if score $slot ui_temp matches 23 run data modify storage ui:gun temp.tmw.gun.SPName set value "イルカライド"
    execute if score $slot ui_temp matches 24 run data modify storage ui:gun temp.tmw.gun.SPName set value "ホップソナー"
...
# 発光表示
    ...
    execute if score $sp ui_temp matches 353 run item modify entity @s enderchest.23 ui:gun/add_glow
    execute if score $sp ui_temp matches 156 run item modify entity @s enderchest.24 ui:gun/add_glow
