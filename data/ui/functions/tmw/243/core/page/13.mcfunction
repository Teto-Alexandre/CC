...
# スロットが空白ならボタンが押されたと判断する＋changedを1に
    ...
    execute if score $slot ui_temp matches 2 run scoreboard players set $sp ui_temp 464
...
# 名前を入力
    execute if score $slot ui_temp matches 2 run data modify storage ui:gun temp.tmw.gun.SPName set value "キューインキ"
