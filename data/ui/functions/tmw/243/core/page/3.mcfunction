...
# スロットが空白ならボタンが押されたと判断する＋changedを1に
    ...
    execute if score $slot ui_temp matches 11 run scoreboard players set $sub ui_temp 207
    execute if score $slot ui_temp matches 12 run scoreboard players set $sub ui_temp 208
...
# 名前を入力
    ...
    execute if score $slot ui_temp matches 11 run data modify storage ui:gun temp.tmw.gun.SubName set value "ヘルハウンドQ"
    execute if score $slot ui_temp matches 12 run data modify storage ui:gun temp.tmw.gun.SubName set value "ブーメランQ"
